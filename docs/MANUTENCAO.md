...
| Etiqueta sem release | Existe uma etiqueta `v*` sem release publicada: a tentativa falhou e ficou a marca. Aconteceu duas vezes com a 1.1.0 (`v1.0.1` e `v1.2.0`, recusadas pelo guarda por não baterem certo com `core/version.py`, e apagadas a 2026-09-21) | `gh release list` contra `git tag`; o guarda é `tools/verificar_versao.py` | A cada release, e na revisão quinzenal | Fundador (manual) — enquanto lá está, a etiqueta órfã faz `git describe` mentir sobre a versão publicada |
...
| Ensaio da release | Correr o `release.yml` sem publicar, para a próxima etiqueta não ser a primeira vez que o caminho é percorrido | **Actions → Release → Run workflow** (`workflow_dispatch`) | Antes de cada release, e sempre que o Dependabot mexer numa Action do `release.yml` | Fundador (manual) — o runner descarrega as Actions antes do primeiro passo, por isso o ensaio apanha um SHA que deixou de existir |
