...
ser dado como configurado. Última varredura de pendências: 2026-09-15.
...
| Log a rodar depressa demais `[pós]` | Existência de `app.log.1`…`app.log.3` renovados no mesmo dia: volume anómalo de `WARNING`/`ERROR` | `core/log.py` (rotativo, 2 MB × 5 ≈ 12 MB no total) | Ao receber um pedido de suporte | Utilizador envia o ficheiro; fundador lê |
...
| Ensaio de atualização preservando dados | Substituir os ficheiros da aplicação como o instalador faz e confirmar que tarefas, configuração e plugins sobrevivem | `tools/testar_atualizacao.py`, no job `ensaios` | Semanal e a pedido (`workflow_dispatch`); obrigatório antes de cada release | CI (automático) |
| Ensaio do instalador de ponta a ponta | Instalação limpa, atualização, desinstalação com e sem `/REMOVEDATA` | `tools/testar_instalador.py`, no job `ensaios` (Inno Setup instalado no runner) | Semanal e a pedido; obrigatório antes de cada release | CI (automático) |
...
| Higiene da cadeia de build | Confirmar que as Actions continuam fixadas por SHA, que os SHA correspondem à versão pretendida e que o workflow continua sem permissões de escrita | `.github/workflows/tests.yml` (Actions fixadas por SHA, `permissions: contents: read`) | Semestral, e sempre que o Dependabot propuser uma Action nova | Fundador (manual) |
